...
(function ($) {
	...
	var Slider = function (element, options) {
		...
		this.reversed =
			this.element.data("slider-reversed") || options.reversed;
	...
	Slider.prototype = {
				...
				this.tooltip[0].style[this.stylePos] =
					(this.size *
						(positionPercentages[0] +
							(positionPercentages[1] - positionPercentages[0]) /
								2)) /
						100 -
		...
		mouseup: function () {
			...
			var val = this.calculateValue();
			this.layout();
			this.element.data("value", val).prop("value", val).trigger({
				type: "slideStop",
				value: val,
			});
			return false;
		...
		getPercentage: function (ev) {
			...
			var percentage =
				((ev[this.mousePos] - this.offset[this.stylePos]) * 100) /
				this.size;
			percentage =
				Math.round(percentage / this.percentage[2]) *
				this.percentage[2];
