...
window.CodeMirror = (function () {
	...
	var opera_version =
		opera && navigator.userAgent.match(/Version\/(\d*\.\d*)/);
	if (opera_version) opera_version = Number(opera_version[1]);
	// Some browsers use the wrong event properties to signal cmd/ctrl on OS X
	var flipCtrlCmd =
		mac &&
		(qtwebkit ||
			(opera && (opera_version == null || opera_version < 12.11)));
	...
	function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
		...
		cm.view.doc.iter(from, to, function (line) {
			if (nextIntact && nextIntact.to == lineNo)
				...
				if (line.widgets && cur.previousSibling)
					for (var i = 0; i < line.widgets.length; ++i)
						if (line.widgets[i].showIfHidden) {
							...
							prev.appendChild(
								buildLineWidget(line.widgets[i], prev, dims)
							);
						}
			} else if (
				nextIntact &&
		...
		if (line.bgClass)
			wrap.appendChild(
				elt(
					"div",
					"\u00a0",
					line.bgClass + " CodeMirror-linebackground"
				)
			);
	...
	function updateSelectionRange(cm) {
		...
		if (sel.from.line == sel.to.line) {
			...
			if (singleLine) {
				...
				else middleTop = heightAtLine(cm, fromObj) - display.viewOffset;
	...
	function restartBlink(cm) {
		...
		display.blinker = setInterval(function () {
			...
			display.cursor.style.visibility =
				display.otherCursor.style.visibility = (on = !on)
					? ""
					: "hidden";
		...
		doc.iter(
			...
			function (line) {
				if (view.frontier >= cm.display.showingFrom) {
					...
					var ischange =
						!oldStyles || oldStyles.length != line.styles.length;
					for (var i = 0; !ischange && i < oldStyles.length; ++i)
						ischange = oldStyles[i] != line.styles[i];
					if (ischange) {
						if (prevChange && prevChange.end == view.frontier)
							prevChange.end++;
						else
							changed.push(
								(prevChange = {
			...
			if ((cur = measure[i])) {
				...
				for (var j = 0; j < vranges.length; j += 2) {
					...
					if (
						(rtop <= top && rbot >= bot) ||
						(top <= rtop && bot >= rbot) ||
						Math.min(bot, rbot) - Math.max(top, rtop) >=
							(bot - top) >> 1
					) {
	...
	function endOperation(cm) {
		...
		if (view.maxLineChanged && !cm.options.lineWrapping) {
			var width = measureChar(
				cm,
				view.maxLine,
				view.maxLine.text.length
			).right;
	...
	function readInput(cm) {
		var input = cm.display.input,
			...
			view = cm.view,
			sel = view.sel;
		if (!view.focused || hasSelection(input) || isReadOnly(cm))
			return false;
	...
	function registerEventHandlers(cm) {
		...
		on(
			...
			operation(cm, function (e) {
				if (
					cm.options.onKeyEvent &&
					cm.options.onKeyEvent(cm, addStop(e))
				)
					return;
		...
		function doSelect(cur) {
			...
			if (type == "double") {
				...
				if (posLess(cur, startstart))
					extendSelection(cm, word.from, startend);
	...
	function onDrop(e) {
		...
		if (files && files.length && window.FileReader && window.File) {
			...
			if (
				...
				!(
					posLess(pos, cm.view.sel.from) ||
					posLess(cm.view.sel.to, pos)
				)
			) {
	...
	function onFocus(cm) {
		...
		if (!cm.view.focused) {
			...
			if (
				cm.display.scroller.className.search(
	...
	function onBlur(cm) {
		if (cm.view.focused) {
			...
			cm.display.scroller.className =
				cm.display.scroller.className.replace(
					" CodeMirror-focused",
					""
				);
		}
	...
	function updateDoc(cm, from, to, newText, selUpdate, origin) {
		...
		var split =
			sawReadOnlySpans && removeReadOnlyRanges(cm.view.doc, from, to);
		if (split) {
		...
		if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == "") {
			...
			if (lines.length == 1) {
				...
				for (var added = [], i = 1, e = lines.length - 1; i < e; ++i)
					added.push(
						makeLine(hlText(lines[i]), hlSpans(lines[i]), th)
					);
				added.push(
					makeLine(
						...
						if (posEq(newPos, curPos)) {
							...
							if (!newPos) {
								if (flipped) {
									...
									if (!mayClear)
										return skipAtomic(cm, pos, bias, true);
	...
	function scrollCursorIntoView(cm) {
		...
		if (doScroll != null && !phantom) {
			...
			if (hidden) {
				...
				display.cursor.style.top =
					coords.top - display.viewOffset + "px";
			}
	...
	function scrollPosIntoView(cm, pos) {
		for (;;) {
			...
			if (scrollPos.scrollLeft != null) {
				...
				if (Math.abs(cm.view.scrollLeft - startLeft) > 1)
					changed = true;
			}
	...
	CodeMirror.prototype = {
		...
		cursorCoords: function (start, mode) {
			...
			if (start == null) pos = sel.head;
				...
				pos = clipPos(this.view.doc, start);
		...
		toggleOverwrite: function () {
			if ((this.view.overwrite = !this.view.overwrite))
				...
				this.display.cursor.className =
					this.display.cursor.className.replace(
		...
		scrollTo: function (x, y) {
			if (x != null)
				this.display.scrollbarH.scrollLeft =
					this.display.scroller.scrollLeft = x;
			if (y != null)
				this.display.scrollbarV.scrollTop =
					this.display.scroller.scrollTop = y;
			updateDisplay(this, []);
		},
		...
		refresh: function () {
			...
			if (this.display.scroller.scrollHeight > sTop)
				this.display.scrollbarV.scrollTop =
					this.display.scroller.scrollTop = sTop;
			if (this.display.scroller.scrollWidth > sLeft)
				this.display.scrollbarH.scrollLeft =
					this.display.scroller.scrollLeft = sLeft;
	...
	var commands = (CodeMirror.commands = {
		...
		goLineStartSmart: function (cm) {
			...
			if (!order || order[0].level == 0) {
				...
				var inWS =
					cur.line == start.line && cur.ch <= firstNonWS && cur.ch;
				cm.extendSelection({
					line: start.line,
	...
	TextMarker.prototype.clear = function () {
		...
		if (this.collapsed && !this.cm.options.lineWrapping)
			for (var i = 0; i < this.lines.length; ++i) {
	...
	function markText(cm, from, to, options, type) {
		...
		doc.iter(curLine, to.line + 1, function (line) {
			if (
				marker.collapsed &&
				!cm.options.lineWrapping &&
				visualLine(doc, line) == cm.view.maxLine
			)
	...
	function addLineWidget(cm, handle, node, options) {
		...
		changeLine(cm, handle, function (line) {
			...
			if (!lineIsHidden(line) || widget.showIfHidden) {
				var aboveVisible =
					heightAtLine(cm, line) < cm.display.scroller.scrollTop;
			...
			if (nextChange == pos) {
				...
				for (var j = 0; j < spans.length; ++j) {
					...
					if (sp.from <= pos && (sp.to == null || sp.to > pos)) {
						...
						if (
							m.collapsed &&
							(!collapsed || collapsed.marker.width < m.width)
						)
							collapsed = sp;
	...
	function lineEnd(cm, lineNo) {
		var merged, line;
		while (
			(merged = collapsedSpanAtEnd((line = getLine(cm.view.doc, lineNo))))
		)
