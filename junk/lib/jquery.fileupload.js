...
(function (factory) {
	...
	$.support.xhrFileUpload = !!(
		window.XMLHttpRequestUpload && window.FileReader
	);
	...
	$.widget("blueimp.fileupload", {
		...
		_BitrateTimer: function () {
			...
			this.getBitrate = function (now, loaded, interval) {
				...
				if (!this.bitrate || !interval || timeDiff > interval) {
					this.bitrate =
						(loaded - this.loaded) * (1000 / timeDiff) * 8;
					this.loaded = loaded;
					this.timestamp = now;
				}
				return this.bitrate;
		...
		_onProgress: function (e, data) {
			if (e.lengthComputable) {
				...
				this._loaded +=
					loaded - (data.loaded || data.uploadedBytes || 0);
				...
				if (options.postMessage) {
					...
					if (options.blob) {
						...
						$.each(options.files, function (index, file) {
							...
							// dummy objects:
							if (
								(window.Blob && file instanceof Blob) ||
		...
		_initIframeSettings: function (options) {
			...
			if (
				options.redirect &&
				$("<a></a>").prop("href", options.url).prop("host") !==
					location.host
			) {
		...
		_getUploadedBytes: function (jqXHR) {
			var range = jqXHR.getResponseHeader("Range"),
				...
				upperBytesPos =
					parts && parts.length > 1 && parseInt(parts[1], 10);
		...
		_chunkedUpload: function (options, testOnly) {
			...
			upload = function () {
				...
				jqXHR = (
					(that._trigger("chunksend", null, o) !== false &&
						$.ajax(o)) ||
		...
		_onAdd: function (e, data) {
			var that = this,
				...
				i;
			if (
				!(options.singleFileUploads || limit) ||
				!this._isXHRUpload(options)
			) {
				fileSet = [data.files];
		...
		_onPaste: function (e) {
			...
			if (
				this._trigger("paste", e, data) === false ||
