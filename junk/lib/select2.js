...
(function ($, undefined) {
	...
	AbstractSelect2 = clazz(Object, {
					...
					if ("ajax" in opts) {
						...
						if (opts.initSelection === undefined) {
							opts.initSelection = function (element, callback) {
								var data = [];
								$(splitVal(element.val(), opts.separator)).each(
									function () {
										var id = this,
											text = this,
											tags = opts.tags;
		...
		monitorSource: function () {
			...
			if (typeof WebKitMutationObserver !== "undefined") {
				...
				this.propertyObserver = new WebKitMutationObserver(function (
					mutations
				) {
		...
		positionDropdown: function () {
			var offset = this.container.offset(),
				...
				enoughRoomAbove =
					offset.top - dropHeight >= this.body().scrollTop(),
		...
		findHighlightableChoices: function () {
			var h = this.results.find(
				".select2-result-selectable:not(.select2-selected):not(.select2-disabled)"
			);
	...
	SingleSelect2 = clazz(AbstractSelect2, {
		...
		setPlaceholder: function () {
			...
			if (this.opts.element.val() === "" && placeholder !== undefined) {
				...
				if (
					this.select &&
					this.select.find("option:first").text() !== ""
				)
					return;
		...
		postprocessResults: function (data, initial) {
			...
			if (initial === true) {
				...
				showSearchInput =
					min < 0 ? false : countResults(data.results) >= min;
		...
		addSelectedChoice: function (data) {
			...
			if (formatted != undefined) {
				choice
					.find("div")
					.replaceWith(
						"<div>" + this.opts.escapeMarkup(formatted) + "</div>"
					);
			}
	...
	$.fn.select2.defaults = {
		...
		formatInputTooShort: function (input, min) {
			...
			return (
				"Please enter " + n + " more character" + (n == 1 ? "" : "s")
			);
		},
		formatInputTooLong: function (input, max) {
			var n = input.length - max;
			return (
				"Please enter " + n + " less character" + (n == 1 ? "" : "s")
			);
		},
