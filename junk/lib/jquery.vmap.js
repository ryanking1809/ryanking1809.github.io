		...
		jQuery(params.container).delegate(
			...
			function (e) {
				if (!params.multiSelectRegion) {
					for (var key in mapData.pathes) {
						map.countries[key].currentFillColor =
							map.countries[key].getOriginalFill();
						map.countries[key].setFill(
							map.countries[key].getOriginalFill()
						);
					}
				}
				...
				jQuery(params.container).trigger("regionClick.jqvmap", [
					code,
					mapData.pathes[code].name,
				]);
			...
			if (typeof key == "string") {
				...
				for (var code in colors) {
					if (this.countries[code]) {
						...
						this.countries[code].setAttribute(
							"original",
							colors[code]
						);
					}
		...
		deselect: function (cc, path) {
			...
			if (this.selectedRegions.indexOf(cc) >= 0) {
				this.selectedRegions.splice(
					this.selectedRegions.indexOf(cc),
					1
				);
				// MUST BE after the change of selectedRegions
				// Otherwise, we might loop
		...
		zoomIn: function () {
			...
			if (map.zoomCurStep < map.zoomMaxStep) {
				...
				jQuery("#zoomSlider").css(
					"top",
					parseInt(jQuery("#zoomSlider").css("top"), 10) - sliderDelta
				);
