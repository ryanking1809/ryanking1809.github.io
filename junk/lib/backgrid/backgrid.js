  ...
  */
		fromRaw: function (number) {
			...
			var decimalPart = parts[1]
				? (this.decimalSeparator || ".") + parts[1]
				: "";

			return (
				integerPart.replace(
					this.HUMANIZED_NUM_RE,
					"$1" + this.orderSeparator
				) + decimalPart
			);
		},
		...
		_convert: function (data, validate) {
			...
			if (this.includeTime) {
				...
				if (this.includeMilli) {
					result =
						result + "." + lpad(jsDate.getUTCMilliseconds(), 3, 0);
				}
			}
  ...
  */
		initialize: function (options) {
			...
			this.editor = this.editor.extend({
				attributes: _.extend(
					{},
					this.editor.prototype.attributes,
		...
		render: function () {
			...
			for (var i = 0; i < optionValues.length; i++) {
				...
				if (_.isArray(optionValue)) {
					...
					optgroup.append(
						this._renderOptions(optionValue.values, currentValue)
					);
  ...
  */
		save: function (e) {
			this.model.set(
				...
				this.formatter.toRaw(this.$el.val())
			);
			this.trigger("done");
		},
	}));
 ...
 */
  ...
  */
   ...
   */
     ...
     If the underlying collection is a Backbone.PageableCollection in
     client-mode, or any
     [Backbone.Collection](https://backbonejs.org/#Collection) instance, sorting
  ...
  */
   ...
   */
		render: function () {
			this.$el.empty();
			var $label = $("<a>")
				.text(this.column.get("label"))
				.append("<b class='sort-caret'></b>");
 ...
 */
   ...
   */
		...
		makeCell: function (column, options) {
			var headerCell =
				column.get("headerCell") || options.headerCell || HeaderCell;
...
*/
  ...
  */
     ...
     [Backbone.Collecton#add](https://backbonejs.org/#Collection-add) event.

     When called directly, it accepts a model or an array of models and an
     option hash just like
     [Backbone.Collection#add](https://backbonejs.org/#Collection-add) and
     ...
     [Backbone.Collection#add](https://backbonejs.org/#Collection-add).
  ...
  */
     ...
     [Backbone.Collection#remove](https://backbonejs.org/#Collection-remove)
     event.

     When called directly, it accepts a model or an array of models and an
     option hash just like
     [Backbone.Collection#remove](https://backbonejs.org/#Collection-remove) and
