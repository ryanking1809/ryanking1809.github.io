...
(function ($, _, Backbone, Backgrid) {
  ...
  */
	Backgrid.Extension.Paginator = Backgrid.Footer.extend({
		...
		/** @property */
		template: _.template(
			'<tr><td colspan="<%= colspan %>"><ul><% _.each(handles, function (handle) { %><li <% if (handle.className) { %>class="<%= handle.className %>"<% } %>><a href="#" <% if (handle.title) {%> title="<%= handle.title %>"<% } %>><%= handle.label %></a></li><% }); %></ul></td></tr>'
		),
    ...
    */
     ...
     */
		changePage: function (e) {
			...
			collection.getPage(
				state.firstPage === 0 ? pageIndex : pageIndex + 1
			);
		},
		...
		makeHandles: function () {
			...
			if (ffLabels) {
				if (ffLabels.prev) {
					handles.unshift({
						label: ffLabels.prev,
						className: collection.hasPrevious()
							? void 0
							: "disabled",
					});
				}
