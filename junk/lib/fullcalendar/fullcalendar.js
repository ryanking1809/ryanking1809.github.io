		...
		function changeView(newViewName) {
			if (!currentView || newViewName != currentView.name) {
				...
				if (currentView) {
					...
					currentView = viewInstances[newViewName] = new fcViews[
						newViewName
					](
	...
	function setMinHeight(element, height) {
		...
		element.each(function (i, _element) {
			_element.style.cssText +=
				";min-height:" + height + ";_height:" + height;
...
-----------------------------------------------------------------------------*/
	...
	function cssKey(_element) {
		return (
			...
			_element.className +
			"/" +
			_element.style.cssText.replace(
...
*/
	...
	function MonthView(element, calendar) {
		...
		function render(date, delta) {
			...
			addDays(
				visStart,
				-((visStart.getDay() - Math.max(firstDay, nwe) + 7) % 7)
			);
			addDays(
				visEnd,
				(7 - visEnd.getDay() + Math.max(firstDay, nwe)) % 7
			);
		...
		function renderDayOverlay(
			...
			for (var i = 0; i < rowCnt; i++) {
				...
				if (stretchStart < stretchEnd) {
					...
					if (rtl) {
						colStart =
							dayDiff(stretchEnd, rowStart) * dis + dit + 1;
						colEnd =
							dayDiff(stretchStart, rowStart) * dis + dit + 1;
					} else {
	...
	--------------------------------------------------------*/
		...
		function dayOfWeekCol(dayOfWeek) {
			return (
				((dayOfWeek - Math.max(firstDay, nwe) + colCnt) % colCnt) *
					dis +
				dit
			);
		}
	...
	-----------------------------------------------------------------------------*/
		...
		function timePosition(day, time) {
			...
			if (slotTop === undefined) {
				slotTop = slotTopCache[slotI] = slotTable.find(
					"tr:eq(" + slotI + ") td div"
				)[0].offsetTop; //.position().top; // need this optimization???
			}
			return Math.max(
				0,
				Math.round(
	...
	---------------------------------------------------------------------------------*/
		...
		function reportDayClick(date, allDay, ev) {
			trigger(
				"dayClick",
				dayBodyCells[dayOfWeekCol(date.getDay())],
				date,
				allDay,
				ev
			);
	...
	----------------------------------------------------------------------------*/
		...
		function compileDaySegs(events) {
			var levels = stackSegs(
					sliceSegs(
						events,
						$.map(events, exclEndDay),
						t.visStart,
						t.visEnd
					)
				),
		...
		function renderSlotSegs(segs, modifiedEventId) {
			...
			for (i = 0; i < segCnt; i++) {
				...
				availWidth = Math.min(availWidth - 6, availWidth * 0.95); // TODO: move this to CSS
				if (levelI) {
					// indented and thin
				...
				left =
					leftmost + // leftmost possible
					(availWidth / (levelI + forward + 1)) *
						levelI * // indentation
						dis +
					(rtl ? availWidth - outerWidth : 0); // rtl
				seg.top = top;
				seg.left = left;
			...
			for (i = 0; i < segCnt; i++) {
				...
				if ((eventElement = seg.element)) {
					...
					seg.vsides =
						val === undefined
							? (vsideCache[key] = vsides(eventElement, true))
							: val;
					val = hsideCache[key];
					seg.hsides =
						val === undefined
							? (hsideCache[key] = hsides(eventElement, true))
							: val;
		...
		function draggableSlotEvent(event, eventElement, timeElement) {
			...
			eventElement.draggable({
				...
				drag: function (ev, ui) {
					minuteDelta =
						Math.round(
							(ui.position.top - origPosition.top) / slotHeight
						) * opt("slotMinutes");
		...
		function resizableSlotEvent(event, eventElement, timeElement) {
			...
			eventElement.resizable({
				...
				resize: function (ev, ui) {
					...
					slotDelta = Math.round(
						(Math.max(slotHeight, eventElement.height()) -
			...
			for (j = 0; j < level.length; j++) {
				...
				for (k = 0; k < levels[i - 1].length; k++) {
					...
					if (segsCollide(segForward, segBack)) {
						segBack.forward = Math.max(
							segBack.forward || 0,
							(segForward.forward || 0) + 1
						);
					}
				}
			}
				...
				hoverListener.start(function (cell, origCell) {
					if (cell) {
						...
						dayDelta =
							...
							c * dis +
							dit -
							(origCell.row * 7 + origCell.col * dis + dit);
						var newEnd = addDays(eventEnd(event), dayDelta, true);
						if (dayDelta) {
							eventCopy.end = newEnd;
							var oldHelpers = helpers;
							helpers = renderTempDaySegs(
								compileDaySegs([eventCopy]),
								seg.row,
								elementTop
							);
							helpers
