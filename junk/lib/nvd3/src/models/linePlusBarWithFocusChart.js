nv.models.linePlusBarWithFocusChart = function () {
	...
	var showTooltip = function (e, offsetElement) {
		...
		var left = e.pos[0] + (offsetElement.offsetLeft || 0),
			...
			y = (e.series.bar ? y1Axis : y2Axis).tickFormat()(
				lines.y()(e.point, e.pointIndex)
			),
	...
	function chart(selection) {
		selection.each(function (data) {
			...
			var wrap = container
				.selectAll("g.nv-wrap.nv-linePlusBar")
				.data([data]);
			var gEnter = wrap
				.enter()
				.append("g")
			...
			if (showLegend) {
				...
				g.select(".nv-legendWrap")
					.datum(
						data.map(function (series) {
							...
							series.key =
								series.originalKey +
								(series.bar ? " (left axis)" : " (right axis)");
							return series;
						})
					)
					.call(legend);
				...
				g.select(".nv-legendWrap").attr(
					"transform",
					"translate(" + availableWidth / 2 + "," + -margin.top + ")"
				);
			}
			...
			function onBrush() {
				...
				g.select(".nv-focus .nv-y1.nv-axis").style(
					"opacity",
					dataBars.length ? 1 : 0
				);
				...
				d3.transition(g.select(".nv-focus .nv-y1.nv-axis")).call(
					y1Axis
				);
				d3.transition(g.select(".nv-focus .nv-y2.nv-axis")).call(
					y2Axis
				);
			}
