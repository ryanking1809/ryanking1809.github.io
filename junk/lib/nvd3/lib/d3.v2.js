(function () {
				  ...
				  Math.floor(
						Math.log(
							x +
								Math.pow(
									10,
									1 + Math.floor(Math.log(x) / Math.LN10) - p
								)
						) / Math.LN10
				  )
				: 1)
	...
	function d3_transition(groups, id, time) {
		...
		groups.ease = function (value) {
			...
			ease =
				typeof value === "function"
					? value
					: d3.ease.apply(d3, arguments);
			return groups;
		};
		groups.each = function (type, listener) {
			if (arguments.length < 2)
				return d3_transition_each.call(groups, type);
		...
		return d3.format(
			",." +
				Math.max(
					0,
					-Math.floor(
						Math.log(d3_scale_linearTickRange(domain, m)[2]) /
							Math.LN10 +
							0.01
					)
				) +
	...
	function d3_scale_pow(linear, exponent) {
		...
		scale.nice = function () {
			return scale.domain(
				d3_scale_nice(scale.domain(), d3_scale_linearNice)
			);
		};
	...
	function d3_scale_ordinal(domain, ranger) {
		function scale(x) {
			return range[
				((index.get(x) || index.set(x, domain.push(x))) - 1) %
					range.length
			];
		}
		...
		scale.rangePoints = function (x, padding) {
			...
			var start = x[0],
				stop = x[1],
				step =
					(stop - start) / (Math.max(1, domain.length - 1) + padding);
			range = steps(
				domain.length < 2
					? (start + stop) / 2
					: start + (step * padding) / 2,
				step
			);
			rangeBand = 0;
			ranger = {
		...
		scale.rangeBands = function (x, padding, outerPadding) {
			...
			var reverse = x[1] < x[0],
				...
				step =
					(stop - start) /
					(domain.length - padding + 2 * outerPadding);
			...
			: points[0] +
					d3_svg_lineHermite(
						...
						d3_svg_lineCardinalTangents(
							[points[points.length - 2]].concat(points, [
								points[1],
							]),
							tension
						)
	...
	function d3_svg_lineFiniteDifferences(points) {
		...
		while (++i < j) {
			m[i] =
				(d + (d = d3_svg_lineSlope((p0 = p1), (p1 = points[i + 1])))) /
				2;
		}
		m[i] = d;
		return m;
	...
	function d3_voronoi_tessellate(vertices, callback) {
		...
		var EventQueue = {
			...
			insert: function (he, site, offset) {
				...
				for (
					var i = 0, list = EventQueue.list, l = list.length;
					i < l;
					i++
				) {
					var next = list[i];
	...
	d3.min = function (array, f) {
		...
		if (arguments.length === 1) {
			...
			while (
				++i < n &&
				((a = f.call(array, array[i], i)) == null || a != a)
			)
				a = undefined;
			while (++i < n)
		...
		for (var j = -1, m = this.length; ++j < m; ) {
			for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
				if ((node = group[i])) {
					subgroups.push(
						(subgroup = d3_array(
							selector.call(node, node.__data__, i)
						))
					);
		...
		if (n < 3) {
			if (typeof name !== "string") {
				...
				for (priority in name)
					this.each(
						d3_selection_style(priority, name[priority], value)
					);
				return this;
			}
			if (n < 2)
	...
	d3_selectionPrototype.on = function (type, listener, capture) {
		...
		if (n < 3) {
			if (typeof type !== "string") {
				...
				for (capture in type)
					this.each(
						d3_selection_on(capture, type[capture], listener)
					);
				return this;
			}
		...
		for (var j = -1, m = this.length; ++j < m; ) {
			...
			for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
				if (
					...
					if ("__data__" in node.node)
						subnode.__data__ = node.node.__data__;
					subgroup.push({
						node: subnode,
						delay: node.delay,
						duration: node.duration,
	...
	d3.behavior.drag = function () {
		function drag() {
			this.on("mousedown.drag", mousedown).on(
				"touchstart.drag",
				mousedown
			);
		}
		function mousedown() {
			...
			var target = this,
				...
				touchId =
					d3.event.touches && d3.event.changedTouches[0].identifier,
				offset,
				origin_ = point(),
				moved = 0;
			var w = d3
		...
		wiggle: function (data) {
			...
			for (j = 1; j < m; ++j) {
				...
				for (i = 0, s2 = 0, dx = x[j][0] - x[j - 1][0]; i < n; ++i) {
					for (
						k = 0,
							s3 = (data[i][j][1] - data[i][j - 1][1]) / (2 * dx);
						k < i;
						++k
					) {
		...
		function tree(d, i) {
			function firstWalk(node, previousSibling) {
				...
				if (children && (n = children.length)) {
					...
					var midpoint =
						0.5 * (firstChild._tree.prelim + child._tree.prelim);
					if (previousSibling) {
						layout.prelim =
		...
		azimuthal.invert = function (coordinates) {
			...
			return [
				(x0 + Math.atan2(x * sc, p * cy0 * cc + y * sy0 * sc)) /
					d3_geo_radians,
				Math.asin(cc * sy0 - (p ? (y * sc * cy0) / p : 0)) /
					d3_geo_radians,
			];
		};
	...
	d3.geo.albers = function () {
		function albers(coordinates) {
			var t = n * (d3_geo_radians * coordinates[0] - lng0),
				p =
					Math.sqrt(
						C - 2 * n * Math.sin(d3_geo_radians * coordinates[1])
					) / n;
			return [
				...
				y =
					-(
						Math.log(
							Math.tan(
								Math.PI / 4 +
									(coordinates[1] * d3_geo_radians) / 2
							)
						) / d3_geo_radians
					) / 360;
			return [
	...
	d3.geo.path = function () {
		...
		function area(coordinates) {
			return Math.abs(
				d3.geom.polygon(coordinates.map(projection)).area()
			);
		}
		var pointRadius = 4.5,
		...
		var pathType = d3_geo_type({
			...
			MultiLineString: function (o) {
				...
				while (++i < n) {
					...
					buffer.push("M");
					while (++j < m)
						buffer.push(project(subcoordinates[j]), "L");
					buffer.pop();
				}
			},
			Polygon: function (o) {
				...
				while (++i < n) {
					...
					while (++j < m) {
						...
						if ((p = subsubcoordinates.length - 1) > 0) {
							buffer.push("M");
							while (++k < p)
								buffer.push(project(subsubcoordinates[k]), "L");
							buffer[buffer.length - 1] = "Z";
						}
					}
				}
	...
	d3.geo.circle = function () {
		...
		function resample(coordinates) {
			var i = 0,
				...
				origin = arc.source();
			while (++i < n) {
				resamples = arc.source(coordinates[i - 1])(
					coordinates[i]
				).coordinates;
		...
		var clipType = d3_geo_type({
			FeatureCollection: function (o) {
				...
				return (
					features &&
					((o = Object.create(o)), (o.features = features), o)
				);
			},
			Feature: function (o) {
				...
				return (
					geometry &&
					((o = Object.create(o)), (o.geometry = geometry), o)
				);
			},
			Point: function (o) {
	...
	var d3_time_days = d3_time_daySymbols,
		...
		d3_time_monthAbbreviations = d3_time_months.map(
			d3_time_formatAbbreviate
		);
	...
	d3_time_daySymbols.forEach(function (day, i) {
		...
		var interval = (d3.time[day] = d3_time_interval(
			...
			function (date) {
				...
				return (
					Math.floor(
						(d3.time.dayOfYear(date) + ((day + i) % 7)) / 7
					) -
					(day !== i)
				);
			}
	...
	var d3_time_scaleLinear = d3.scale.linear(),
		d3_time_scaleLocalFormat = d3_time_scaleFormat(
