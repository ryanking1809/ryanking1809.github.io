/*
 * Downloaded from https://d3js.org/d3.v3.js, 2013-02-12 15:38
 */
...
d3 = (function () {
	...
	d3.xhr = function (url, mimeType, callback) {
		...
		["get", "post"].forEach(function (method) {
			xhr[method] = function () {
				return xhr.send.apply(
					xhr,
					[method].concat(d3_array(arguments))
				);
			};
		});
	...
	if (d3_format_grouping) {
		...
		d3_format_group = function (value) {
			...
			while (i > 0 && g > 0) {
				...
				g =
					d3_format_grouping[
						(j = (j + 1) % d3_format_groupingLength)
					];
			}
	...
	function d3_ease_elastic(a, p) {
		...
		return function (t) {
			return (
				1 + a * Math.pow(2, 10 * -t) * Math.sin(((t - s) * 2 * π) / p)
			);
		};
	}
	...
	var d3_mouse_bug44083 = /WebKit/.test(d3_window.navigator.userAgent)
		? -1
		: 0;
	function d3_mousePoint(container, e) {
		...
		if (svg.createSVGPoint) {
			...
			if (
				d3_mouse_bug44083 < 0 &&
				(d3_window.scrollX || d3_window.scrollY)
			) {
				svg = d3
	...
	d3.svg.axis = function () {
		...
		axis.orient = function (x) {
			...
			orient =
				x in d3_svg_axisOrients ? x + "" : d3_svg_axisDefaultOrient;
			return axis;
		};
		axis.ticks = function () {
	...
	d3.behavior.drag = function () {
		...
		function mousedown() {
			...
			var w = d3
				...
				.on(
					touchId != null
						? "touchmove.drag-" + touchId
						: "mousemove.drag",
					dragmove
				)
				.on(
					touchId != null
						? "touchend.drag-" + touchId
						: "mouseup.drag",
					dragend,
					true
				);
		...
		d3_behavior_zoomWheel =
			"onwheel" in document
				? ((d3_behavior_zoomDelta = function () {
						return (
							-d3.event.deltaY * (d3.event.deltaMode ? 120 : 1)
						);
				  }),
				  "wheel")
	...
	d3.geo.centroid = function (object) {
		...
		if (
			...
			return [
				...
				Math.asin(Math.max(-1, Math.min(1, d3_geo_centroidZ / m))) *
					d3_degrees,
			];
		}
	};
		...
		function clipLine(listener) {
			...
			return {
				...
				point: function (λ, φ) {
					...
					if (
						v &&
						(!point0 || !d3_geo_sphericalEqual(point0, point1))
					)
	...
	d3.geo.greatArc = function () {
		...
		greatArc.source = function (_) {
			...
			interpolate =
				source_ && target_
					? d3.geo.interpolate(source_, target_)
					: null;
			return greatArc;
		};
	...
	function d3_geo_mercator(λ, φ) {
		return [
			...
			Math.max(
				-0.5,
				Math.min(+0.5, Math.log(Math.tan(π / 4 + φ / 2)) / (2 * π))
			),
		];
	}
	...
	function d3_time_zone(d) {
		var z = d.getTimezoneOffset(),
			...
			zm = Math.abs(z) % 60;
		return (
			zs + d3_time_formatPad(zh, "0", 2) + d3_time_formatPad(zm, "0", 2)
		);
	}
