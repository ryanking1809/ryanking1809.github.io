...
!(function ($) {
			...
			for (var i in $element[0].attributes) {
				...
				if (
					...
					if (
						...
						return true;
					obj[this.camelize(attribute.name.replace(namespace, ""))] =
	...
	(function (exports) {
		...
		Constraint.prototype = {
			...
			check: function (object, group) {
				...
				for (var property in this.options.strict
					? this.nodes
					: object) {
					if (
						this.options.strict
		...
		Assert.prototype = {
			...
			Callback: function (fn) {
				...
				this.fn = fn;
				this.validate = function (value) {
					var result = this.fn.apply(
						this,
						[value].concat(this.arguments)
					);
					if (true !== result)
			...
			Count: function (count) {
				...
				this.validate = function (array) {
					...
					if (isNaN(Number(count)))
						throw new Error(
							"Count must be a valid interger",
							count
						);
	...
	ParsleyValidator.prototype = {
		...
		addValidator: function (name, fn, priority) {
			this.validators[name.toLowerCase()] = function (requirements) {
				return $.extend(
					new Validator.Assert().Callback(fn, requirements),
					{ priority: priority }
				);
			};
			return this;
		...
		validators: {
			...
			min: function (value) {
				return $.extend(
					new Validator.Assert().GreaterThanOrEqual(value),
					{
						priority: 30,
						requirementsTransformer: function () {
							return "string" === typeof value && !isNaN(value)
			...
			range: function (array) {
				return $.extend(
					new Validator.Assert().Range(array[0], array[1]),
					{
						priority: 32,
						requirementsTransformer: function () {
							for (var i = 0; i < array.length; i++)
								array[i] =
									"string" === typeof array[i] &&
									!isNaN(array[i])
										? parseInt(array[i], 10)
										: array[i];
							return array;
	...
	ParsleyUI.prototype = {
		...
		manageErrorsMessages: function (fieldInstance, diff) {
			if (
				"undefined" !==
				typeof fieldInstance.options.errorsMessagesDisabled
			)
				return;
		...
		addError: function (
			...
			fieldInstance._ui.$errorsWrapper.addClass("filled").append(
				$(fieldInstance.options.errorTemplate)
					...
					.html(
						message || this._getErrorMessage(fieldInstance, assert)
					)
			);
		...
		focus: function (formInstance) {
			...
			for (var i = 0; i < formInstance.fields.length; i++)
				if (
					...
					"undefined" ===
						typeof formInstance.fields[i].options.noFocus
				) {
					if ("first" === formInstance.options.focus) {
						formInstance._focusedField =
							formInstance.fields[i].$element;
		...
		_diff: function (newResult, oldResult, deep) {
			...
			return {
				...
				added: added,
				removed: !deep
					? this._diff(oldResult, newResult, true).added
					: [],
			};
		},
		...
		_insertErrorWrapper: function (fieldInstance) {
			...
			if ("function" === typeof fieldInstance.options.errorsContainer)
				$errorsContainer =
					fieldInstance.options.errorsContainer(fieldInstance);
			if (
				"undefined" !== typeof $errorsContainer &&
				$errorsContainer.length
			)
				return $errorsContainer.append(
					fieldInstance._ui.$errorsWrapper
				);
	...
	var ConstraintFactory = function (
		...
		if (
			...
			requirements =
				window.ParsleyValidator.validators[name](
					requirements
				).requirementsTransformer();
		return $.extend(
			window.ParsleyValidator.validators[name](requirements),
	...
	$.emit = function (name, instance) {
		...
		for (var i = 0; i < subscribed[name].length; i++) {
			...
			if (
				...
				for (
					var j = 0;
					j < subscribed[name][i].instance.fields.length;
					j++
				)
					if (
						subscribed[name][i].instance.fields[j].__id__ ===
						instance.__id__
					) {
						subscribed[name][i].fn.apply(
							o,
	...
	window.ParsleyConfig.i18n.en = $.extend(
	...
	Parsley.prototype = {
		...
		handleMultiple: function (parsleyFormInstance) {
			...
			this.options = $.extend(
				this.options,
				parsleyFormInstance
					? parsleyFormInstance.OptionsFactory.get(
							parsleyFormInstance
					  )
					: {},
	...
	window.ParsleyUI =
		"function" ===
		typeof ParsleyUtils.get(window, "ParsleyConfig.ParsleyUI")
