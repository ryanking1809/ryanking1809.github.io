...
(function ($) {
		...
		if (xhr.readyState > 0) {
			if (options.push && !options.replace) {
				...
				window.history.pushState(
					null,
					"",
					stripPjaxParam(options.requestUrl)
				);
			}
	...
	function extractContainer(data, xhr, options) {
		...
		obj.url = stripPjaxParam(
			xhr.getResponseHeader("X-PJAX-URL") || options.requestUrl
		);

		// Attempt to parse response html into elements
		if (/<html/i.test(data)) {
			var $head = $(
				parseHTML(data.match(/<head[^>]*>([\s\S.]*)<\/head>/i)[0])
			);
			var $body = $(
		...
		if (options.fragment) {
			...
			if ($fragment.length) {
				...
				if (!obj.title)
					obj.title =
						$fragment.attr("title") || $fragment.data("title");
			}
