...
function initPjax() {
	var PjaxApp = function () {
		...
		if (this.pjaxEnabled) {
			...
			$(document).on(
				"pjax:start",
				$.proxy(this._resetResizeCallbacks, this)
			);
			...
			$(document).on(
				"pjax-app:loaded",
				$.proxy(this._loadingFinished, this)
			);
	...
	PjaxApp.prototype.extractPageName = function (url) {
		//credit: https://stackoverflow.com/a/8497143/1298418
		var pageName = url
			.split("#")[0]
		...
		.on("click", function () {
			if ($(window).width() < 768) {
				...
				if (!$secondLevelMenu.is(".in")) {
					...
					$content.css(
						"margin-top",
						contentMargin - subMenuHeight + "px"
					);
				}
			}
		});
...
!(function ($) {
	...
	Widgster.prototype.close = function () {
		//could have been remove.widgster, but https://bugs.jquery.com/ticket/14600
		...
		this.options.closePrompt &&
			this.options.closePrompt($.proxy(this._doClose, this));
	...
	Widgster.prototype.load = function () {
		...
		this.$element
			.find(this.options.bodySelector)
			.load(this.options.load, function (responseText, textStatus, xhr) {
				widgster.expand();
				widgster.options.showLoader && widgster._hideLoader();
				widgster.$element.trigger(
					$.Event("loaded.widgster", {
	...
	Widgster.prototype._showLoader = function () {
		...
		if (this.$loaderWrap.length == 0) {
			this.$loaderWrap = $(
				'<div class="widgster-loader-wrap" style="position: absolute; top: 0; right: 0; bottom: 0; ' +
	...
	Widgster.prototype._hideLoader = function () {
		...
		this.$element
			.find(this.options.bodySelector)
			.fadeTo(this.options.transitionDuration, 1);
	};
	...
	Widgster.prototype._initHandlers = function () {
		...
		this.$element.on(
			"click.fullscreen.widgster",
			"[data-widgster=fullscreen]",
			$.proxy(this.fullscreen, this)
		);
		this.$element.on(
			"click.restore.widgster",
			"[data-widgster=restore]",
			$.proxy(this.restore, this)
		);
	};
	...
	function getNamespacedData(namespace, data) {
		...
		for (var key in data) {
			...
			if (key.slice(0, namespace.length) == namespace) {
				...
				namespacedKey =
					namespacedKey.charAt(0).toLowerCase() +
					namespacedKey.slice(1);
	...
	$.fn.widgster = function (option) {
		return this.each(function () {
			...
			var options = $.extend(
				{},
				Widgster.DEFAULTS,
				getNamespacedData("widgster", $this.data()),
