...
var dummyBodies = [
	...
	"somm text bodt. Reall small. ust few lines",
...
$(function () {
	function pageLoad() {
		...
		var EmailList = Backbone.Collection.extend({
			...
			search: function (word, folderId) {
				...
				return this.filter(function (mail) {
					return (
						(mail.get("folderId") == folderId &&
							pat.test(mail.get("subject"))) ||
						pat.test(mail.get("sender"))
					);
				});
			},
		});
		...
		var EmailOpenedView = Backbone.View.extend({
			...
			replyEmail: function () {
				var body =
					...
					newEmail = new Email({
						body: body,
						receiver: this.model.get("sender")
							? this.model.get("sender")
							: this.model.get("senderMail"),
		...
		var ComposeView = Backbone.View.extend({
			...
			events: {
				"click #compose-save-button, #compose-send-button, #compose-discard-button":
					"backToFolders",
			},
			...
			_initViewComponents: function () {
				var customWysihtml5Templates = {
					"font-styles": function (locale) {
						return (
							...
							"<i class='fa fa-font'></i>&nbsp;<span class='current-font'>" +
							...
							locale.font_styles.normal +
							"</a></li>" +
							"<li><a data-wysihtml5-command='formatInline' data-wysihtml5-command-value='span'>" +
							locale.font_styles.normal +
							"</a></li>" +
					...
					lists: function (locale) {
						return (
							"<li>" +
							"<div class='btn-group'>" +
							"<a class='btn btn-sm btn-inverse ' data-wysihtml5-command='insertUnorderedList' title='" +
							locale.lists.unordered +
							"'><i class='fa fa-list'></i></a>" +
							"<a class='btn btn-sm btn-inverse ' data-wysihtml5-command='insertOrderedList' title='" +
							locale.lists.ordered +
							"'><i class='fa fa-th-list'></i></a>" +
							"<a class='btn btn-sm btn-inverse ' data-wysihtml5-command='Outdent' title='" +
							locale.lists.outdent +
							"'><i class='fa fa-outdent'></i></a>" +
							"<a class='btn btn-sm btn-inverse ' data-wysihtml5-command='Indent' title='" +
							locale.lists.indent +
					...
					link: function (locale) {
						return (
							...
							"</div>" +
							"</div>" +
							"<a class='btn btn-sm btn-inverse ' data-wysihtml5-command='createLink' title='" +
							locale.link.insert +
					...
					image: function (locale) {
						return (
							...
							"</div>" +
							"</div>" +
							"<a class='btn btn-sm btn-inverse ' data-wysihtml5-command='insertImage' title='" +
							locale.image.insert +
					...
					html: function (locale) {
						return (
							"<li>" +
							"<div class='btn-group'>" +
							"<a class='btn btn-sm btn-inverse ' data-wysihtml5-action='change_view' title='" +
							locale.html.edit +
				...
				this.$("#email-body").wysihtml5({
					...
					events: {
						load: function () {
							$(this.composer.doc.head).prepend(
		...
		var EmailListView = Backbone.View.extend({
			...
			folderActionsTemplate: _.template(
				$("#folder-actions-template").html()
			),
			...
			initialize: function () {
				...
				this.listenTo(
					this.currentFolderEmails,
					"reset",
					this.renderEmails
				); //when displayed first time, and after
				this.listenTo(
					this.currentFolderEmails,
					"all",
					this.renderFolderActions
				); //when something starred or selected display folder actions
				this.listenTo(
					this.currentFolderEmails,
					"destroy",
					this.renderEmails
			...
			renderFolderActions: function () {
				var selectedCount = this.currentFolderEmails.where({
						selected: true,
					}).length,
					allSelected =
						selectedCount == this.currentFolderEmails.length,
					anySelected = selectedCount > 0;
				this.$("#folder-actions").html(
					this.folderActionsTemplate({
			...
			renderEmails: function () {
				...
				var currentFolder = this.folders.where({ current: true })[0],
					unreadCount = this.currentFolderEmails.where({
						read: false,
					}).length,
			...
			resetEmails: function () {
				...
				if (folderId == STARRED_FOLDER_ID) {
					this.currentFolderEmails.reset(
						Emails.where({
							starred: true,
						})
					);
			...
			toggleAll: function () {
				...
				this.currentFolderEmails.each(function (email) {
					email.save({ selected: selectAll });
				});
			},
			...
			selectRead: function () {
				this.selectNone();
				_(this.currentFolderEmails.where({ read: true })).each(
					function (email) {
						email.save({ selected: true });
					}
				);
			},
			...
			selectUnread: function () {
				this.selectNone();
				_(this.currentFolderEmails.where({ read: false })).each(
					function (email) {
						email.save({ selected: true });
					}
				);
			...
			search: function () {
				...
				this.currentFolderEmails.reset(
					Emails.search($("#mailbox-search").val(), folderId)
				);
			},

			markSelectedAsRead: function () {
				_(this.currentFolderEmails.where({ selected: true })).each(
					function (email) {
						email.save({ read: true });
					}
				);
			},
